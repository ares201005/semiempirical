...
def sort_atom_list(mol):
    atom_charges = mol.atom_charges()
    print("atom_charges:", atom_charges)
    list_1 = []
    list_2 = []
    for ia in range(mol.natm):
    ...
    print("atom_list_sorted:", atom_list_sorted)
...
def get_hcore_mndo_sorted(mol, model, python_integrals, params, atom_list_sorted):
    ...
    atom_charges = mol.atom_charges()
    basis_atom_charges = atom_charges[mol._bas[:,gto.ATOM_OF]]
    print("basis_atom_charges:", basis_atom_charges)
...
class RMNDO(scf.hf.RHF):
    ...
    def __init__(self, mol, model, python_integrals=0, atom_sorted=1):
    ...
    def get_hcore(self, mol=None):
        if self._mndo_model == 'MINDO3':
           ...
           if self._atom_sorted != 1:
              ...
              self.atom_list_sorted = sort_atom_list(self._mndo_mol)
              return get_hcore_mndo_sorted(self._mndo_mol, self._mndo_model, self.python_integrals, self.params, self.atom_list_sorted)
    ...
    def get_jk(self, mol=None, dm=None, hermi=1, with_j=True, with_k=True):
        ...
        if self._mndo_model == 'MINDO3':
            ...
            if self._atom_sorted != 1:
                ...
                return get_jk_mndo_sorted(self._mndo_mol, dm, self._mndo_model, self.python_integrals, self.params, self.atom_list_sorted)
...
def _get_gamma(mol, F03=None): #From mindo3.py -CL
    ...
    rho = np.array([mopac_param.E2/F03[z] for z in atom_charges]) #g_ss was MOPAC_AM
